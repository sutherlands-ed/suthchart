  ...
  renderElement: (e) ->
    switch e.type
      ...
      when 'text'
        ...
        """<text x="#{e.x}" y="#{e.y}" style="#{e.style}" text-anchor="#{e.textAnchor}" stroke="none" fill="#{e.color}" font-size="#{e.fontSize}px" font-family="#{e.fontFamily}"><tspan dy="#{e.fontSize * 0.35}">#{e.text}</tspan></text>"""
      else
