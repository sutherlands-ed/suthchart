class VMLRenderer
  ...
  renderElement: (e) ->
    switch e.type
      ...
      when 'text'
        """<rvml:shape class="rvml" coordsize="1,1" style="position:absolute;width:1px;height:1px;top:0px;left:0px;" filled="t" fillcolor="#{e.color}" stroked="f" path="m#{Math.round(e.x - 1)},#{Math.round(e.y)}r1,0e"><rvml:stroke class="rvml" opacity="1" miterlimit="8"></rvml:stroke><rvml:textpath class="rvml" style="font-family=#{e.fontFamily};font-size:#{e.fontSize}px;v-text-align:center;v-text-kern:true" on="t" string="#{e.text}"></rvml:textpath><rvml:path class="rvml" textpathok="t"></rvml:path><rvml:skew class="rvml" on="t" matrix="1,0,0,1,0,0" offset="-.5,-.5"></rvml:skew><rvml:fill class="rvml" type="solid"></rvml:fill></rvml:shape>"""
      else
