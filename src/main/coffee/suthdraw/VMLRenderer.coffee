class VMLRenderer extends suthdraw.Renderer
  ...
  renderElement: (e) ->
    ...
    switch e.type
      when 'circle'
        """<v:oval #{VMLRenderer.idIfSet(e)}class="sd-circle" style="position:absolute;left:#{crisp(e.x-e.r)}px;top:#{crisp(e.y-e.r)}px;width:#{e.r * 2}px;height:#{e.r * 2}px#{VMLRenderer.styleAddition(e)}" strokecolor="#{e.strokeColor}" fillcolor="#{e.fillColor}"><v:stroke opacity="#{e.opacity}" miterlimit="8"></v:stroke><v:fill type="solid" opacity="#{e.opacity}"></v:fill></v:oval>"""
      when 'curve'
