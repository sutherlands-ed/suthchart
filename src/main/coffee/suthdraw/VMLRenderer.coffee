    ...
    switch e.type
      ...
      when 'line'
        if (e.crispEdges)
          ...
          """<rvml:line #{e.idIfSet()}class="rvml sd-line" from="#{e.x1}px,#{e.y1}px" to="#{e.x2}px,#{e.y2}px" strokecolor="#{e.strokeColor}" strokeweight="#{e.strokeWidth}px"><rvml:stroke class="rvml" opacity="#{e.strokeWidth}" miterlimit="8"></rvml:stroke></rvml:line>"""
      when 'oval'
        """<rvml:oval #{e.idIfSet()}class="rvml sd-oval" style="position:absolute;left:#{e.x-e.rx+0.5}px;top:#{e.y-e.ry+0.5}px;width:#{e.rx * 2}px;height:#{e.ry * 2}px;" strokecolor="#{e.strokeColor}" fillcolor="#{e.fillColor}"><rvml:stroke class="rvml" opacity="#{e.opacity}" miterlimit="8"></rvml:stroke><rvml:fill class="rvml" type="solid" opacity="#{e.opacity}"></rvml:fill></rvml:oval>"""
      when 'text'
