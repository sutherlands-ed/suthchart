class VMLRenderer extends suthdraw.Renderer
  ...
  render: (drawing) ->
    html = []
    html.push("""<div style="clip:rect(0px #{drawing.width}px #{drawing.height}px 0px); position:relative; width:#{drawing.width}px;display:inline-block;height:#{drawing.height}px;overflow:hidden;top:0px; left:0px;cursor:crosshair">""")
  ...
  renderElement: (e) ->
    ...
    switch e.type
      ...
      when 'oval'
        """<v:oval #{VMLRenderer.idIfSet(e)}class="sd-oval" #{VMLRenderer.style(e,"position:absolute;left:#{crisp(e.x-e.rx)}px;top:#{crisp(e.y-e.ry)}px;width:#{e.rx * 2}px;height:#{e.ry * 2}px")}strokecolor="#{e.strokeColor}" fillcolor="#{e.fillColor}"><v:stroke opacity="#{e.opacity}" miterlimit="8"></v:stroke><v:fill type="solid" opacity="#{e.opacity}"></v:fill></v:oval>"""
      when 'path'
